...
class Role(db.Model):
    ...
    @staticmethod
    def insert_roles():
        # 三个角色：普通用户（默认角色）、协管员、管理员
        # dic 里 key 是角色的 name 属性值， value 是各类权限之和
        roles = {
        ...
        for r, v in roles.items():
            # 查询角色数据表里是否有此数据，如果没有则新建实例
            # 如果有的话，后面的代码被跳过
            role = Role.query.filter_by(name=r).first()
            if role is None:
                role = Role(name=r)
            role.permissions = v
            # 如果角色的 name 属性为 'User'，则其 default 属性值为 True
            # 否则为 False
...
class User(db.Model, UserMixin):
    ...
    def generate_confirm_user_token(self):
        ...
        Args:
            expires_in: Token 的过期时间（秒），默认 1 小时
        """
